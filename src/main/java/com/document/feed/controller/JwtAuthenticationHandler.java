...
public class JwtAuthenticationHandler {
  ...
  public RouterFunction authRoute() {
    return RouterFunctions.route(POST("/auth/signin").and(accept(APPLICATION_JSON))
        .and(accept(APPLICATION_JSON)), this::signIn)
  ...
  public Mono<ServerResponse> signIn(ServerRequest request) {
    ...
    return jwtRequestMono.flatMap(
        jwtRequest -> userRepository.findByUsername(jwtRequest.getUsername()).flatMap(
            user -> ServerResponse.ok().contentType(APPLICATION_JSON).body(BodyInserters
                .fromValue(new JwtResponse(generateToken(user))))).switchIfEmpty(
  ...
  private Mono<ServerResponse> signUp(final ServerRequest request) {
    ...
    return userMono.flatMap(user -> userRepository.findByUsername(user.getUsername()).flatMap(
        dbUser -> ServerResponse.badRequest()
            .body(BodyInserters.fromValue("User already exist"))).switchIfEmpty(
  ...
  public String generateToken(User user) {
    System.out
        .println("Start generateToken(): " + user.getUsername() + "," + user.getPassword());
    ...
    JwtBuilder builder = Jwts.builder().setClaims(claims)
        ...
        .signWith(SignatureAlgorithm.HS256, jwtTokenUtil.getSecret()).setExpiration(
            new Date(
